import{$ as e,M as b,x as L,i as F,d as y,a as p,j as f,k as u,N as c,y as U,T as q,z as R,p as M,b as _,q as E,c as N,r as P,A as B,t as G,e as H,h as A,v as V,o as W}from"./biwin-BZFQmYYA.js";let l=!1,r=[];async function h(){try{const a=M("wiFestividades");if(a&&!l)return x(a);r=(await _(E(N(p,"festividades"),P("orden")))).docs.map(i=>({id:i.id,...i.data()})),u("wiFestividades",r,24),x(r)}catch(a){console.error("Error festividades:",a),c("Error al cargar festividades","error")}}function x(a){window.festividadesGlobal=a;const t=`
  ...
  `;e(".miweb").replaceWith(t)}e(document).on("click","#alternarModo",function(){l=!l,h(),b(l?"Modo Edición activado ✏️":"Modo Vista activado 👁️")});e(document).on("click","#botonAgregarFestividad",()=>z());e(document).on("click",".boton-editar",function(){const a=r.find(t=>t.id===e(this).data("id"));a&&z(a)});e(document).on("click",".boton-eliminar",async function(){const a=e(this).data("id"),t=r.find(i=>i.id===a);L({titulo:"¿Eliminar Festividad?",mensaje:`¿Estás seguro de eliminar "${t?.nombre}"? Esta acción no se puede deshacer.`,tipo:"danger",textoConfirmar:"Eliminar",onConfirmar:async()=>{try{await B(y(p,"festividades",a)),r=r.filter(i=>i.id!==a),u("wiFestividades",r,24),h(),b("Festividad eliminada ✅")}catch(i){console.error("Error eliminando:",i),c("Error al eliminar","error")}}})});e(document).on("blur",'[contenteditable="true"]',async function(){if(!l)return;const t=e(this).closest(".fest_card").data("id"),i=e(this).data("field"),n=e(this).text().trim();if(!(!t||!i))try{const s=r.find(o=>o.id===t);if(!s)return;s[i]=n,await F(y(p,"festividades",t),{[i]:n,actualizado:f()},{merge:!0}),u("wiFestividades",r,24),c(`${i} actualizado ✅`,"success",2e3)}catch(s){console.error("Error guardando inline:",s),c("Error al guardar","error")}});function z(a=null){const t=a!==null,i=o=>o?.toDate?new Date(o.toDate().getTime()-o.toDate().getTimezoneOffset()*6e4).toISOString().split("T")[0]:"",n=o=>o?JSON.stringify(o,null,2):"{}",s=`
