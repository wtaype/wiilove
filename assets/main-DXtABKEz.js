const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["assets/primavera-BtnOco0d.js","assets/biwin-Bzp6bOmS.js","assets/biwin-DiF5g45u.css","assets/primavera-DF34mVJt.css"])))=>i.map(i=>d[i]);
import{$ as a,r as P,t as q,d as S,a as _,i as k,q as E,j,w as $,u as V,m as M,v as B,x as G,s as W,b as Y,M as y,e as x,y as H,z as J,A as Z,p as O,h as F,o as K,l as Q,n as X}from"./biwin-Bzp6bOmS.js";function aa(){const t=`
...
<div id="recuperarModal" class="modal authModals">
  <div class="modal-content">
    ...
    ${O()}
  `),await ea()}async function ea(){try{const t=F("wisfesti");if(t)return window.festividadesGlobal=t,L(t);const i=(await k(E(j(_,"festividades"),K("orden")))).docs.map(d=>({id:d.id,...d.data()})).filter(d=>d.activo!==!1);window.festividadesGlobal=i,x("wisfesti",i,24),L(i)}catch(t){console.error("Error displayPublico:",t),a("#fest-grid-publico").html('<p style="text-align:center;color:var(--error);font-size:1.2rem;">Error al cargar festividades</p>')}}function L(t){const l=t.map((i,d)=>(i.tags?.includes("Primaveral")||i.tags?.includes("Romántico")||i.tags?.includes("Audio")||i.tags?.includes("Celebración")||i.tags?.includes("Sorpresa")||i.tags?.includes("Paternal")||i.tags?.includes("Maternal")||i.tags?.includes("Navideño")||i.tags?.includes("Renovación"),`
      <div class="fest_card anim_up" data-fest="${i.nombreId}" style="animation-delay:${d*.1}s">
        <div class="card_head">
...
<main class="miweb miwp">
    ...
    <div id="notifications-container"></div>
    ${O()}
  `)}const ia="modulepreload",ta=function(t){return"/wiilove/"+t},N={},R=function(l,i,d){let c=Promise.resolve();if(i&&i.length>0){let m=function(v){return Promise.all(v.map(p=>Promise.resolve(p).then(w=>({status:"fulfilled",value:w}),w=>({status:"rejected",reason:w}))))};document.getElementsByTagName("link");const f=document.querySelector("meta[property=csp-nonce]"),e=f?.nonce||f?.getAttribute("nonce");c=m(i.map(v=>{if(v=ta(v),v in N)return;N[v]=!0;const p=v.endsWith(".css"),w=p?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${v}"]${w}`))return;const h=document.createElement("link");if(h.rel=p?"stylesheet":ia,p||(h.as="script"),h.crossOrigin="",h.href=v,e&&h.setAttribute("nonce",e),document.head.appendChild(h),p)return new Promise((A,s)=>{h.addEventListener("load",A),h.addEventListener("error",()=>s(new Error(`Unable to preload CSS for ${v}`)))})}))}function u(f){const e=new Event("vite:preloadError",{cancelable:!0});if(e.payload=f,window.dispatchEvent(e),!e.defaultPrevented)throw f}return c.then(f=>{for(const e of f||[])e.status==="rejected"&&u(e.reason);return l().catch(u)})},oa=(t,l,i)=>{const d=t[l];return d?typeof d=="function"?d():Promise.resolve(d):new Promise((c,u)=>{(typeof queueMicrotask=="function"?queueMicrotask:setTimeout)(u.bind(null,new Error("Unknown variable dynamic import: "+l+(l.split("/").length!==i?". Note that variables only represent file names one level deep.":""))))})};async function ra(t){let l=t.get("fest")||window.location.search.replace("?","").split("&")[0];const i=t.get("de")||"Amigos",d=t.get("para")||"Amiga",c=t.get("msg")||"main",u=t.get("txt");if(!l||l.includes("="))return window.location.href="/";try{const f=F("wisfesti");let e=f?.find(p=>p.alias===l||p.nombreId===l);if(!e){const p=await k(E(j(_,"festividades"),$("alias","==",l)));p.empty||(e={id:p.docs[0].id,...p.docs[0].data()},x("wisfesti",[...f||[],e],24))}if(!e)return window.location.href="/";const m=c==="custom"&&u?u:e.mensajes?.[c]||e.mensajes?.main,v=await oa(Object.assign({"./parati/anuevo.js":()=>R(()=>Promise.resolve().then(()=>ca),void 0),"./parati/cumple.js":()=>R(()=>Promise.resolve().then(()=>la),void 0),"./parati/navidad.js":()=>R(()=>Promise.resolve().then(()=>na),void 0),"./parati/primavera.js":()=>R(()=>import("./primavera-BtnOco0d.js"),__vite__mapDeps([0,1,2,3]))}),`./parati/${e.plantilla||"default"}.js`,3);a(".app").html(v.default(e,i,d,m,e.audioUrl))}catch(f){console.error("Error parati:",f),window.location.href="/"}}const D=new URLSearchParams(window.location.search);D.toString()?ra(D):(Q(M,async t=>{if(!t)return sa();try{const l=F("wiSmile");if(l){I(l);return}const d=(await k(E(j(_,"smiles"),$("usuario","==",t.displayName)))).docs[0]?.data()||{};x("wiSmile",d,450),I(d)}catch(l){console.error(l)}}),a(document).on("click",".bt_salir",async()=>{await X(M),window.location.href="/";try{localStorage.clear()}catch{Object.keys(localStorage).forEach(l=>localStorage.removeItem(l))}}));const ca=Object.freeze(Object.defineProperty({__proto__:null},Symbol.toStringTag,{value:"Module"})),la=Object.freeze(Object.defineProperty({__proto__:null},Symbol.toStringTag,{value:"Module"})),na=Object.freeze(Object.defineProperty({__proto__:null},Symbol.toStringTag,{value:"Module"}));
